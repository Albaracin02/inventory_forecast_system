...
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $cat_id = $conn->real_escape_string($row['cat_id']);
        $year_id = 6;
        $forecastQuantity = $conn->real_escape_string($forecast_quantity_2024);
        $forecastCost = $conn->real_escape_string($forecast_cost_2024);
        $forecastTotalCost = $conn->real_escape_string($forecast_total_cost_2024);

        $insertSql = "INSERT INTO fact_forecast (forecast_id, item_id, cat_id, year_id, quantity, unit_cost, total_cost)
                    VALUES ('$count', '$item_id', '$cat_id', '$year_id', '$forecastQuantity', '$forecastCost', '$forecastTotalCost')";

        if ($conn->query($insertSql) !== TRUE) {
            echo "Error: " . $insertSql . "<br>" . $conn->error;
            $success = false; // Set success flag to false if any insertion fails
            break; // Exit the loop if there's an error to prevent further insert attempts
        }

        $count++;
        }

        // Check if all insertions were successful before redirecting
        if ($success) {
        header("Location: buttonforecast.php");
        exit(); // Ensure subsequent code doesn't execute after the redirect
    }
} else {
